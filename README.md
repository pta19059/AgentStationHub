...
[User] ──► [Blazor Server: AgentChatPanel]
          ...
          [Foundry Agent Service — AgenticStationFoundry / project=default]
                   ...
                   │   ├─ openapi tool: microsoft_learn_catalog → Logic App `REDACTED-LOGICAPP`
...
- **Tools live in the portal, not in the code**: `microsoft_learn_catalog` (Logic App `REDACTED-LOGICAPP`, anonymous SAS) and the Microsoft Learn MCP server are attached to `AgentMicrosoftLearn` in the Foundry portal. Editing tools / instructions / model never requires re-deploying the Hub.
...
```jsonc
{
  "Foundry": {
    ...
    "ChatAgent": {                     // Agent Learn (floating MS-logo chat)
      ...
      "LearnToolUrl": "https://REDACTED-LOGICAPP-HOST/api/Get_Learning_Content/triggers/Request/invoke?api-version=...&sig=...",
...
- **Pre-pin Azure tenant + subscription from the Hub UI** ([`AgentStationHub/Components/Pages/Hub.razor`](AgentStationHub/Components/Pages/Hub.razor), [`AgentStationHub/Components/DeploymentModal.razor`](AgentStationHub/Components/DeploymentModal.razor), [`AgentStationHub/Services/DeploymentOrchestrator.cs`](AgentStationHub/Services/DeploymentOrchestrator.cs), [`AgentStationHub/Services/Tools/SandboxAzureAuth.cs`](AgentStationHub/Services/Tools/SandboxAzureAuth.cs), [`AgentStationHub/Models/DeploymentSession.cs`](AgentStationHub/Models/DeploymentSession.cs)). New optional **Azure tenant id** + **Azure subscription id** fields on the Hub filters row let the user pin the identity for the deploy up-front. Both are GUIDs, validated in `Orchestrator.NormalizeGuid` (a stray paste of a domain name or display name is silently dropped instead of being forwarded to `az login --tenant ...` where it would cause a confusing "tenant not found" loop). Values are persisted in the browser's `localStorage` (`agentichub.auth.tenantId` / `agentichub.auth.subscriptionId`) so the user types them once, not per-deploy; deliberately client-side because the Hub host can be shared across users and the server should not become a credential directory. Plumbing: `DeploymentSession` gained `TenantId` / `SubscriptionId` (init-only), `Orchestrator.Start` now takes them as optional params, and `RunAsync` forwards the session's `TenantId` to `SandboxAzureAuth.EnsureAsync` ahead of the legacy `ReadHostTenantId()` host-profile fallback. `SandboxAzureAuth.EnsureAsync` now (a) accepts an explicit `subscriptionId`, (b) parses the cached `az account show -o json` output and **wipes the cached profile** (`az logout` + best-effort `rm -rf /root/.azure/msal_token_cache.* azureProfile.json accessTokens.json service_principal_entries.json`) when its `tenantId` does not match the user's pin — that is the central fix for the "wrong default tenant" loop, where a previous deploy's stale cache would silently win and azd would deploy into the wrong directory, (c) runs `az account set --subscription <id>` after login when `subscriptionId` was supplied (with a warn-and-continue when the id is invalid for the tenant), and (d) no longer short-circuits on `_verified` when the caller passed an explicit tenant or subscription, so a UI identity switch always re-validates. Net effect: a user with multiple AAD tenants can specify the right one in the UI and the very first device-code login lands on it, eliminating the manual `az logout` + retry loop the orchestrator could not previously break out of on its own.
- **`DeployIntentExtractor`: hardened against the Quick-Start trap** ([`AgentStationHub.SandboxRunner/Team/PlanningTeam.cs`](AgentStationHub.SandboxRunner/Team/PlanningTeam.cs)). Tightened the extractor's instructions after a real run on `leestott/On-Call-Copilot-Multi-Agent` showed the LLM stopping at the first 5 KB of the README ("Quick Start") instead of reading the full 21 KB end-to-end and finding "Deploy to Microsoft Foundry → Option A". New rule 0 mandates an end-to-end README read; rule 2 enumerates deploy-section headings to look for (`Deploy to Microsoft Foundry`, `Deploy to Azure`, `Deploy`, `Deployment`, `Production deployment`, `Run on Azure`, `Provision and deploy`); rule 3 makes the `infra/main.bicep` emptiness check mandatory (`< 10` lines or zero `resource ...` declarations is the strongest signal that scaffolding via `azd init -t <template>` / `azd ai agent init` is required); rule 7 calibrates confidence explicitly (`0.0-0.4` when only Quick Start was found ⇒ `commands=[]`, never promote local-dev to deploy). Exclusion list expanded with `.venv` activation, `pip install -r requirements.txt`, `npm install`, `bash scripts/start.sh` and any localhost-bound command. The output JSON gained two verifiability fields, `readmeSectionsScanned` (the headings actually found) and `deploySectionHeading` (which one was used as the source) so a future log-trace shows whether the extractor stopped early.
- **`DeployIntentExtractor` agent: README-grounded canonical deploy procedure** ([`AgentStationHub.SandboxRunner/Team/PlanningTeam.cs`](AgentStationHub.SandboxRunner/Team/PlanningTeam.cs)). Until now the planning pipeline saw only structural signals — file detector + tree + the first 2.4 KB of every `infra/*.sh` — and NEVER read the README itself. That is why on `leestott/On-Call-Copilot-Multi-Agent` the Strategist correctly emitted `azd up` from the structural signals (azure.yaml + Dockerfile + agent.yaml + infra/main.bicep present) but missed the **bootstrap sequence** the README documents under "Deploy to Microsoft Foundry → Option A": `azd init -t https://github.com/Azure-Samples/azd-ai-starter-basic` then `azd ai agent init -m agent.yaml` then `azd up`. The repo's `infra/main.bicep` is intentionally a **30-byte stub** (`targetScope = 'resourceGroup'`); the real template is materialised by `azd ai agent init` from `agent.yaml`. Without the bootstrap step ARM rejects the deploy with `parsing ARM template: ARM template contains no resources` and the Doctor + EscalationResolver have no in-sandbox patch (because the fix is "scaffold from an upstream template" — not a sed-able local change). The new `DeployIntentExtractor` is a tool-using LLM agent (Agent Framework, `read_workspace_file` + `list_workspace_directory` from the existing `DoctorToolbox`) that runs AFTER `Scout` and BEFORE `TechClassifier` in `PlanningTeam.RunAsync`. Its instructions tell it to read the README's "Deploy" / "Deployment" / "Run on Azure" / "Get started" sections in full, treat any scaffolding command (`azd init -t <template>`, `azd ai agent init`, `terraform init`, `sam deploy --guided`) as MANDATORY before the main deploy, and emit strict JSON `{ commands: [{cmd, source, sourceLine, purpose}], prereqs, envVars, notes, confidence }`. The output is validated as JSON-with-`commands`-array and injected into `BuildStrategistInput` as a banner-style `CANONICAL DEPLOY INTENT (extracted from this repo's README + manifests)` block when non-empty. The Strategist's instructions inside that banner are unambiguous: "When confidence >= 0.7 you MUST reproduce them VERBATIM, in the listed order, as the FIRST steps of your plan." Adaptations are explicitly limited to (a) stripping interactive prompts (`--no-prompt`/`--yes`), (b) substituting placeholder values, (c) preferring the baked sandbox helpers (`agentic-azd-up` over raw `azd up`). Hallucinating commands not in the JSON is forbidden. All failure modes are non-fatal: missing README, network error, malformed LLM output, or `confidence < 0.7` ⇒ the block is skipped and the Strategist falls back to pure manifest inference (the legacy behaviour). Net effect: the orchestrator now reads the deploy doc the way a human operator would — once at planning time — instead of arguing with the Doctor about ARM error tails for 8-13 attempts trying to retro-fit a procedure that the README spelled out on line 1.
