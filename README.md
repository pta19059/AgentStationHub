...
[User] ──► [Blazor Server: AgentChatPanel]
          ...
          [FoundryAgentChatClient.CallLearnAsync]
                   │  POST https://REDACTED-LOGICAPP-HOST/api/Get_Learning_Content/
                   │       triggers/Request/invoke?api-version=...&sig=... (anonymous SAS)
                   ▼
          [Logic App Standard `REDACTED-LOGICAPP` (Sweden Central)]
...
- **Agent Learn — floating chat avatar in the Hub sidebar** ([`AgentChatPanel.razor`](AgentStationHub/Components/AgentChatPanel.razor), [`FoundryAgentChatClient.cs`](AgentStationHub/Services/Tools/FoundryAgentChatClient.cs), wiring in [`Program.cs`](AgentStationHub/Program.cs)). MS-logo avatar opens a chat powered by Azure OpenAI `gpt-4.1-mini-1` with a single function-calling tool (`get_learning_content`) that fetches the live Microsoft Learn catalog through Logic App Standard `REDACTED-LOGICAPP`. Foundry V2 Responses returned 404 on cluster `hyena-swedencentral-02` (preview), so the agent runs in-process via the chat-completions data plane on the AgenticStationFoundry account. `?` tooltip next to the title surfaces the architecture summary inline.
