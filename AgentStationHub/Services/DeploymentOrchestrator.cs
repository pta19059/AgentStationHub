                    ...
                    else
                    {
                        ...
                        if (cosmosFailedState)
                        {
                            ...
                            var cleanupScript = string.Join("\n", new[]
                            {
                                ...
                                "# Only delete cosmos accounts that are in failed/deleting state or that are -aif- variants.",
                                "# Do NOT delete healthy (Succeeded) main cosmos accounts.",
                                "DELETED_NAMES=''",
                                "for n in $ALL_NAMES; do",
                                "  STATE=$(az cosmosdb show -g \"$RG\" -n \"$n\" --query 'provisioningState' -o tsv 2>/dev/null || echo 'NotFound')",
                                "  echo \"  $n state=$STATE\"",
                                "  if [ \"$STATE\" = 'Succeeded' ] && ! echo \"$n\" | grep -q '\\-aif-'; then",
                                "    echo \"  SKIPPING $n (healthy main cosmos, not -aif-)\"",
                                "    continue",
                                "  fi",
                        ...
                        if (embeddingQuota)
                        {
                            var quotaScript = string.Join("\n", new[]
                            {
                                ...
                                "# 0. Switch text-embedding-3-large SKU from Standard to GlobalStandard in main.parameters.json.",
                                "#    The Standard quota for OpenAI.Standard.text-embedding-3-large in eastus is",
                                "#    typically 350 TPM and gets saturated by orphan deployments from other repos;",
                                ...
                                "    j=json.loads(data)",
                                "except Exception as e:",
                                "    print('json parse failed: %s' % e); sys.exit(0)",
                                "params=j.get('parameters',{})",
                                "mdl=params.get('modelDeploymentList',{}).get('value',[])",
                                "changed=False",
                                "for d in mdl:",
                                "    m=d.get('model',{})",
                                "    if m.get('name')=='text-embedding-3-large':",
                                "        sku=d.setdefault('sku',{})",
                                "        if sku.get('name')!='GlobalStandard':",
                                "            print('switching text-embedding-3-large sku %s -> GlobalStandard' % sku.get('name'))",
                                ...
                                "if [ -z \"$ACCS\" ]; then echo 'no AOAI/AIServices accounts in sub'; else",
                                ...
                                "az cognitiveservices usage list --location \"$LOC\" --query \"[?contains(name.value,'text-embedding-3-large')].{name:name.localizedValue,quota:limit,used:currentValue}\" -o table 2>/dev/null || true",
                        ...
                        if (rtSignal && !rtJustRan)
                        {
                            ...
                            if (isRealtimeRepo)
                            {
                                ...
                                var pyScript = string.Join("\n", new[]
                                {
                                    ...
                                    "    # 2b. Normalise sku.name inside any realtime deployment block:",
                                    "    #     sku name must be a CAPACITY sku ('GlobalStandard'), NOT",
                                    "    #     the model name. Many Azure-Samples bicep templates use",
                                    "    #     `sku: { name: <sameParam> }` which collapses model+sku to",
                                    "    #     the model name and trips InvalidResourceProperties.",
                                    "    sku_pattern = re.compile(r\"^(\\s*name:\\s*)'?([^'\\n}]+)'?\\s*$\")",
                                    "    for i, line in enumerate(lines):",
                                    "        # Detect we're on a sku.name line: previous non-blank line",
                                    "        # must contain 'sku:' and current line matches name: <X>.",
                                    "        if 'sku:' not in (lines[i-1] if i > 0 else ''):",
                                    "            continue",
                                    "        nearby = '\\n'.join(lines[max(0,i-6):i+6])",
                                    "        if not (any(p in nearby for p in RT_VERSION_PARAMS)",
                                    "                or 'realtime' in nearby.lower()):",
                                    "            continue",
                                    "        m2 = sku_pattern.match(line)",
                                    "        if not m2:",
                                    "            continue",
                                    "        # Replace whatever the value is with 'GlobalStandard'.",
                                    "        lines[i] = m2.group(1) + \"'GlobalStandard'\"",
                                    "    # 2c. Also force any param ending in 'Sku' / 'SkuName' that lives",
                                    "    #     in a realtime context to GlobalStandard.",
                                    "    for sku_param in (",
                                    "        'realtimeDeploymentSku', 'realtimeDeploymentSkuName',",
                                    "        'openAiRealtimeDeploymentSku', 'realtimeSkuName',",
                                    "    ):",
                                    "        content_tmp = '\\n'.join(lines)",
                                    "        content_tmp = re.sub(",
                                    "            rf\"param\\s+{re.escape(sku_param)}\\s+string(\\s*=\\s*'[^']*')?\",",
                                    "            f\"param {sku_param} string = 'GlobalStandard'\",",
                                    "            content_tmp)",
                                ...
                                var rtScript = string.Join("\n", new[]
                                {
                                    ...
                                    "azd env set realtimeDeploymentSku 'GlobalStandard' 2>/dev/null || true",
                                    "azd env set realtimeSkuName 'GlobalStandard' 2>/dev/null || true",
